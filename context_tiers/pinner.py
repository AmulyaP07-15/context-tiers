...
SIGNAL_WORDS = {"must", "not", "never", "only", "dont",
                "mustnt", "wont", "avoid"}
SIGNAL_PHRASES = ("instead of", "make sure", "do not", "don't", "has to",
                  "have to", "same booking", "without", "n't")
...
class HeuristicPinner:
    def extract(self, user_message: str) -> list[str]:
        ...
        for sentence in (user_message or "").replace("!", ".").split("."):
            ...
            # a bare contraction like "don't" tokenizes as don + t, so also
            # check the raw apostrophe forms
            if "n't" in low:
                hit = True
            if hit:
                pins.append(s)
